...
function cardHTML(article, idx = 0) {
    ...
    return `
    <a class="news-card" data-url="${article.url}" style="animation-delay:${idx * 0.06}s"
      ...
      <div class="card-body">
        ...
        <div class="card-footer">
          ...
          <div class="card-actions">
            <button class="card-action-btn" onclick="event.preventDefault(); event.stopPropagation(); shareArticle('${article.url}','${title.replace(/'/g, "\\'")}')" title="Share">${shareIcon()}</button>
            <button class="card-action-btn bm-btn ${bm ? 'bookmarked' : ''}"
              onclick="event.preventDefault(); event.stopPropagation(); toggleBookmark(${JSON.stringify(article).replace(/"/g, '&quot;')})"
...
function renderBreaking() {
    ...
    breakingCarousel.innerHTML = arts.map((a, i) => {
        ...
        return `<a class="breaking-card" href="${a.url}" target="_blank" rel="noopener noreferrer">
...
function openArticle(article) {
    // Use a real anchor click to work on all mobile browsers (avoids popup blocking)
